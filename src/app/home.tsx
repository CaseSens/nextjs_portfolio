...
export default function Home() {
  ...
  return (
    <div className="transition-colors duration-[2000ms] relative z-0 bg-transparent h-screen grid grid-cols-page-padding-cols-sm place-items-center p-6 md:px-8 lg:px-12 xl:px-16 w-full text-black dark:text-white font-rubik font-bold text-center shrink-0 select-none">
      <img
        src="/waves-bg.svg"
        className="absolute inset-0 object-cover h-full w-full z-[1] dark:opacity-0"
        alt="Background"
      />
      <img
        src="/waves-bg-dark.svg"
        className="absolute inset-0 object-cover h-full w-full z-[0] opacity-0 dark:opacity-100"
        alt="Background"
      />
      <div className="absolute col-start-2 col-end-2 w-full z-30 inset-0 flex items-center justify-center">
        <div className="box-border flex flex-col gap-12 items-center w-full bg-white/[.1] px-6 py-8 md:py-12 rounded-lg backdrop-filter backdrop-blur-sm shadow-2xl px-4 sm:px-8 md:px-12 py-4">
